$(function () {
     ...
     */
    var Obj = {};
    var luckyResult = window.localStorage.getItem('lucky');
    if (luckyResult){
        luckyResult = JSON.parse(luckyResult);
    }else{
        luckyResult = [];
    }
    Obj.alreadyResult = luckyResult;
    Obj.luckyResult = [];
    Obj.luckyPrize = '';
    ...
    */
    ...
    function randomLuckyArr() {
        console.log('luckyNum...', Obj.luckyNum);
        Obj.luckyResult = [];
        for (var i = 0; i < Obj.luckyNum; i++) {
            var random = Math.floor(Math.random() * personArray.length);
            if (Obj.luckyResult.indexOf(random) == -1 && Obj.alreadyResult.indexOf(random) == -1) {
                Obj.luckyResult.push(random)
                Obj.alreadyResult.push(random)
                window.localStorage.setItem('lucky', JSON.stringify(Obj.alreadyResult));
            } else {
                i--;
            }
        }
